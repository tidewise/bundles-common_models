...
class OroGen::Icp::Relocalization < Syskit::Composition
    ...
    event :start do |context|
        ...
        %i[position cov_position orientation cov_orientation].each do |field|
    ...
    on :initial_estimate do |context|
        ...
        Robot.info @pose_sample.position.to_s
        ...
        Orocos.conf.apply(icp_task, %w[default uniform_sampling], true)
        @refinement = icp_task.sendop(:calculateLastPointCloud, @pose_sample, 0)
    end

    poll do
        if initial_estimate?
            status, = @refinement.collect_if_done
            ...
            if (pose = @orientation_reader.read) && icp_task.running?
                ...
                unless initial_pose.respond_to?(:orientation)
